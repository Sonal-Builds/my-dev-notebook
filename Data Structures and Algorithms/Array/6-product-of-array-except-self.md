...
✅ So we use a trick:
We break the product at index i into:
```javascript
result[i] = (product of elements before i) * (product of elements after i)

//🔁 Using Prefix and Suffix as math containers:
prefix[i] = a₀ * a₁ * ... * a_{i-1}
...
```
In the optimized version, instead of using two extra arrays, we:

Store prefix in result (in first pass)

Multiply with running suffix from end (in second pass)
