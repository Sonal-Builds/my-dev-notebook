...
- ✅ `bind()` - Use when you want to store a new function with a fixed this context, to call later.
```javascript
...
- ✅ `call()` - ✅ Use when you want to invoke a function immediately, setting the this value..
```javascript
const person = { name: "Alice" };
...
- ✅ `apply()` - apply() – Like call(), but Takes Arguments as an Array.
```javascript
const person = { name: "Alice" };
